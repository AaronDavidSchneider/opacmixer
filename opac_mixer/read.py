...
class ReadOpac:
    ...
    def setup_temp_and_pres(self, temp=None, pres=None):
        ...
        self.kcoeff = interp_2d(self.T, self.p, temp, pres, self.kcoeff, self.ls, self.lf[0], self.lg[0], self.lt,
                                self.lp, lt_new[0], lp_new[0])
...
class ReadOpacChubb(ReadOpac):
    def __init__(self, files) -> None:
        ...
        self._files = files
        # read meta data:
        lp, lt, lf, lg = np.empty(ls, dtype=int), np.empty(ls, dtype=int), np.empty(ls, dtype=int), np.empty(ls,
                                                                                                             dtype=int)

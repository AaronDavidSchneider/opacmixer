...
def mix_kappa_deep_set(kappas, weights, input_scaling, inverse_output_scaling, *args):
    ...
    kout = inverse_output_scaling(kappas_resh, simple_deep_set(input_scaling(kappas_resh), weights)).T.reshape(shape)
    return kout
...
class PatchedRadtrans(Radtrans):
    ...
    def setup_mixing(self, mixmethod='rorr', weights=None, input_scaling=None, inverse_output_scaling=None):
        """
        ...
        if mixmethod == 'deepset':
            if input_scaling is None:
                input_scaling = t_x
                print("Warning: Default scaling is used for the input, change if needed")
            if inverse_output_scaling is None:
                inverse_output_scaling = ti_y
                print("Warning: Default scaling is used for the output, change if needed")
            if weights is None:
    ...
    def mix_kappa(self, kappas, g):
        if self._mixmethod == 'deepset':
            ...
            return mix_kappa_deep_set(kappas, self._ml_weights, self._ml_input_scaling, self._ml_inverse_output_scaling)
    ...
    def mix_opa_tot(self, abundances, mmw, gravity,
        ...
        if (self.mode == 'c-k') and self.test_ck_shuffle_comp:
            self.line_struc_kappas[:, :, 0, :] = self.line_struc_kappas[:, :, 0, :] - self.continuum_opa
            self.line_struc_kappas[:, :, 0, :] = self.mix_kappa(self.line_struc_kappas, gravity)
            self.line_struc_kappas[:, :, 0, :] = self.line_struc_kappas[:, :, 0, :] + self.continuum_opa
    ...
    def calc_bolometric_flux(self, temp, abunds, gravity, mmw, sigma_lnorm=None, \
        ...
        t = time.time()
        self.mix_opa_tot(abundances=abunds, mmw=mmw, gravity=gravity, sigma_lnorm=sigma_lnorm, fsed=fsed, Kzz=Kzz,
                         radius=radius, \
    ...
    def calc_RT_bolometric(self):
        ...
        self.flux_bol, self.flux_star = fs_bol.feautrier_rad_trans_bolometric(self.border_freqs, \
                                                                              self.total_tau[:, :, 0, :], \
                                                                              self.temp, \
                                                                              self.mu, \
                                                                              self.w_gauss_mu, \

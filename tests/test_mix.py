...
def test_mix_single(setup_interp_reader):
    ...
    assert np.all(
        np.isclose(lm, rm)
    ), "mixes are not the same, even though we just mix one species!"
    assert np.all(
        np.isclose(lm, opac.kcoeff[mol_single_i, :, :])
    ), "we do not get the correct mix!"
...
def RandOverlap_2_kdata_prof(Nlay, Nw, Ng, kdata1, kdata2, weights, ggrid):
    ...
    for ilay in range(Nlay):
        for iW in range(Nw):
            if (kdata1[ilay, iW, -1] - kdata2[ilay, iW, 0]) * (
                kdata2[ilay, iW, -1] - kdata1[ilay, iW, 0]
            ) < 0.0:
                # ii+=1
...
def test_individual_rorr_vs_grid_rorr(setup_test_mix_grid, setup_test_mix_ind):
    ...
    mix_ind_resh = mix_ind.reshape(
        (opac_grid.lt[0], opac_grid.lp[0], opac_grid.lf[0], opac_grid.lg[0])
    ).transpose(1, 0, 2, 3)
    ...
    for p in [1e-1]:
        for t in [2000]:
            ...
            for fi in range(opac.lf[0]):
                x = opac.bin_edges[fi] + opac.weights.cumsum() * (
...
def test_mix_vs_prt(setup_test_mix_grid):
    ...
    for i, temp in enumerate(opac.Tr):
        ...
        np.testing.assert_allclose(
            atmosphere.line_struc_kappas[:, :, :, :],
            opac.kcoeff[:, :, i, ::-1, :].transpose(3, 2, 0, 1),
            rtol=0.01,
        )

        atmosphere.mix_opa_tot(
            abunds,
            mmw,
            g,
            sigma_lnorm,
...
def _test_mix_vs_exok(setup_test_mix_grid):
    ...
    for i, temp in enumerate(opac.Tr):
        kcoeff_exok[:, i, :, :] = (
            mmr[0, :, i, np.newaxis, np.newaxis] * opac.kcoeff[0, :, i, :, :]
        )
        for speci in range(1, len(opac.spec)):
            # (Nlay, Nw, Ng)
            kdata2 = (
                mmr[speci, :, i, np.newaxis, np.newaxis]
                * opac.kcoeff[speci, :, i, :, :]
            )
            kdata1 = kcoeff_exok[:, i, :, :]

            kcoeff_exok[:, i, :, :] = RandOverlap_2_kdata_prof(
                opac.lp[0],
                opac.lf[0],
                opac.lg[0],
                kdata1,

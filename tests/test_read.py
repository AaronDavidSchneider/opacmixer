...
def test_interp_edges(setup_reader):
    ...
    assert np.all(
        np.isclose(opac_temp.kcoeff[:, 0, 0, :, :], opac.kcoeff[:, 0, 0, :, :])
    )
    ...
    for speci in range(opac.ls):
        assert np.all(
            np.isclose(
                opac_temp.kcoeff[speci, 0, 0, :, :],
                opac.kcoeff[speci, opac.lp[speci] - 1, 0, :, :],
            )
        )
    ...
    for speci in range(opac.ls):
        assert np.all(
            np.isclose(
                opac_temp.kcoeff[speci, 0, 0, :, :],
                opac.kcoeff[speci, 0, opac.lt[speci] - 1, :, :],
            )
        )
    ...
    for speci in range(opac.ls):
        assert np.all(
            np.isclose(
                opac_temp.kcoeff[speci, 0, 0, :, :],
                opac.kcoeff[speci, opac.lp[speci] - 1, opac.lt[speci] - 1, :, :],
            )
        )
...
def test_interp_vs_prt(setup_interp_reader):
    ...
    for i, temp in enumerate(opac.Tr):
        ...
        ), "interpolation gave different result"
